...
Rather then calling  the scanner class in each class that expected input I decided to use  an Input handler  class than controls all the types of input expected throughout the whole system and also adde a try catch in that class to prevent errors I also  added a try catch in App.menu within the assigndriver function to prevent any error while accepting,completing or any other error during the ride and also locations .java to prevent the user choosing non -existent locations
